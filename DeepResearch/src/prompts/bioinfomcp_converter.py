"""
BioinfoMCP Converter prompts for generating MCP servers from bioinformatics tools.

This module contains prompts for converting command-line bioinformatics tools
...
# Prompt templates for BioinfoMCP operations
BIOINFOMCP_PROMPTS: dict[str, str] = {
    "system": BIOINFOMCP_SYSTEM_PROMPT,
    "convert_tool": "Convert the following bioinformatics tool documentation to MCP server code: {tool_documentation}",
    "generate_server": "Generate MCP server code for {tool_name} with the following documentation: {documentation}",
    "validate_conversion": "Validate the MCP server code for {tool_name}: {server_code}",
}


class BioinfoMCPConverterPrompts:
    """Prompt templates for BioinfoMCP converter operations."""

    SYSTEM = BIOINFOMCP_SYSTEM_PROMPT
    PROMPTS = BIOINFOMCP_PROMPTS
