...
class AgentBuilderConfig(BaseModel):
    ...
    subagents: List[Union[SubAgent, CustomSubAgent]] = Field(default_factory=list, description="Subagents")
    middleware_config: Dict[str, Any] = Field(default_factory=dict, description="Middleware configuration")
    enable_parallel_execution: bool = Field(True, description="Enable parallel execution")
...
class AgentGraphExecutor:
    ...
    async def _execute_graph_traversal(self, execution_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while current_node:
            ...
            if not self._dependencies_satisfied(dependencies, execution_state):
                ...
                current_node = self._handle_dependency_wait(current_node, execution_state)
                continue
    ...
    async def _execute_node(
        ...
        try:
            ...
            return {"success": False, "error": "timeout", "execution_time": execution_time}
    ...
    def _get_next_node(
        ...
        for node in adjacent_nodes:
            if (node not in execution_state["completed_nodes"] and 
                node not in execution_state["failed_nodes"]):
                return node
...
class AgentBuilder:
    ...
    def build_graph(self, nodes: List[AgentGraphNode], edges: List[AgentGraphEdge]) -> AgentGraph:
        """Build an agent graph."""
        return AgentGraph(
            nodes=nodes,
            edges=edges,
            entry_point=nodes[0].name if nodes else "",
            exit_points=[node.name for node in nodes if not self._has_outgoing_edges(node.name, edges)]
        )
...
def create_deep_agent(
    ...
    **kwargs
    ...
    default_tools = ["write_todos", "list_files", "read_file", "write_file", "edit_file", "task"]
