...
class BaseDeepAgent:
    ...
    def _update_metrics(self, execution_time: float, success: bool, tools_used: List[str]) -> None:
...
class PlanningAgent(BaseDeepAgent):
    ...
    async def create_plan(self, task_description: str, context: Optional[DeepAgentState] = None) -> AgentExecutionResult:
...
class FilesystemAgent(BaseDeepAgent):
    ...
    async def manage_files(self, operation: str, context: Optional[DeepAgentState] = None) -> AgentExecutionResult:
...
class ResearchAgent(BaseDeepAgent):
    ...
    async def conduct_research(self, research_query: str, context: Optional[DeepAgentState] = None) -> AgentExecutionResult:
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        if config is None:
            config = AgentConfig(
                ...
                capabilities=[AgentCapability.TASK_ORCHESTRATION, AgentCapability.PLANNING]
            )
        super().__init__(config)
    
    async def orchestrate_tasks(self, task_description: str, context: Optional[DeepAgentState] = None) -> AgentExecutionResult:
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        if config is None:
            config = AgentConfig(
                ...
                tools=["write_todos", "list_files", "read_file", "write_file", "edit_file", "task"],
                capabilities=[
...
def create_task_orchestration_agent(config: Optional[AgentConfig] = None) -> TaskOrchestrationAgent:
    """Create a task orchestration agent."""
    return TaskOrchestrationAgent(config)


def create_general_purpose_agent(config: Optional[AgentConfig] = None) -> GeneralPurposeAgent:
