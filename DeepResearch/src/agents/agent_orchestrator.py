...
class NestedLoopRequest(BaseModel):
    ...
    break_conditions: List[BreakCondition] = Field(default_factory=list, description="Break conditions")
    state_machine_mode: MultiStateMachineMode = Field(MultiStateMachineMode.GROUP_CHAT, description="State machine mode")
    subgraphs: List[SubgraphType] = Field(default_factory=list, description="Subgraphs to include")
    agent_roles: List[AgentRole] = Field(default_factory=list, description="Agent roles")
...
class OrchestrationResult(BaseModel):
    ...
    nested_loops_spawned: List[str] = Field(default_factory=list, description="Nested loops spawned")
    subgraphs_executed: List[str] = Field(default_factory=list, description="Subgraphs executed")
...
- Can spawn agents: {self.config.can_spawn_agents}"""
            ...
            try:
                ...
                for condition in self.config.break_conditions:
                    ...
                    if result.should_break:
                        should_break = True
                        break_reason = f"Break condition met: {condition.condition_type.value}"
                        break
    ...
    def _spawn_nested_loop(self, config: NestedReactConfig, deps: OrchestratorDependencies) -> Dict[str, Any]:
        """Spawn a nested REACT loop."""
    ...
    def _execute_subgraph(self, config: SubgraphConfig, deps: OrchestratorDependencies) -> Dict[str, Any]:
        """Execute a subgraph."""
