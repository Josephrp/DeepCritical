...
class FilesystemState(BaseModel):
    ...
    files: Dict[str, FileInfo] = Field(default_factory=dict, description="Files in the filesystem")
    current_directory: str = Field("/", description="Current working directory")
    permissions: Dict[str, List[str]] = Field(default_factory=dict, description="File permissions")
...
class DeepAgentState(BaseModel):
    ...
    def get_planning_state(self) -> PlanningState:
        ...
        return PlanningState(
            todos=self.todos,
            planning_context=self.shared_state.get("planning", {})
        )
...
def merge_filesystem_state(current: Dict[str, FileInfo], update: Dict[str, FileInfo]) -> Dict[str, FileInfo]:
...
def merge_conversation_history(current: List[Dict[str, Any]], update: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
